...
function navigate(page) {
  ...
  if (page === 'messages') {
    ...
    fetchStorageInfo().then(function() { if (msgStorageMode !== 'none') loadMessages(); });
    if (msgRefreshInterval) clearInterval(msgRefreshInterval);
    msgRefreshInterval = setInterval(function() { if (state.page === 'messages' && msgStorageMode !== 'none') loadMessages(); }, 10000);
  } else {
    if (msgRefreshInterval) { clearInterval(msgRefreshInterval); msgRefreshInterval = null; }
  }
}
...
function renderMessageGrid(msgs) {
  ...
  var statusCls = {
    ...
    'FAILED':      'bg-red-50 dark:bg-red-400/10 text-red-600 dark:text-red-400 border-red-200 dark:border-red-400/20',
